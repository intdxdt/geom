...
func initKnn() {
	...
	knnData = make([]mono.MBR, 0, len(dat))
	for i := range dat {
		knnData = append(knnData, mono.MBR{MBR: dat[i]})
	}
}
...
func TestRtreeKNNPredScore(t *testing.T) {
	g := goblin.Goblin(t)
	g.Describe("Index Knn - Pred, Score", func() {
...
func fnRichData() []*mono.MBR {
	...
	for i := range data {
		richData = append(richData, &mono.MBR{MBR: data[i]})
	}
	return richData
}
...
//	g := goblin.Goblin(t)
//	g.Describe("Index Knn Predicate", func() {
